// ============================================================================
// ESQUADЯO — CEP Extension JS Engine (Lápis Raro)
// ============================================================================
...
btnCreate.addEventListener('click', () => {
  ...
  const jsonStr = JSON.stringify(fmt);
  const scriptStr = `createArtboardsFromJSON('${jsonStr.replace(/'/g, "\\'")}', ${count})`;

  csInterface.evalScript(scriptStr, (res) => {
    showToast(`✅ ${count} prancheta(s) criada(s) no Illustrator!`);
  });
});

// Ação 2: Aplicar Apenas Guias
btnGuidesOnly.addEventListener('click', () => {
  const platformKey = platformSelect.value;
  const formats = masterPlatforms[platformKey] || [];
  const fmt = formats.find(f => f.id === formatSelect.value) || formats[0];

  const jsonStr = JSON.stringify(fmt);
  const scriptStr = `applyGuidesToActive('${jsonStr.replace(/'/g, "\\'")}')`;

  csInterface.evalScript(scriptStr, (res) => {
    if (res === "NO_DOC") {
      showToast("⚠️ Abra um documento antes de aplicar as guias.");
    } else {
